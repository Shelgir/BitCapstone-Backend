...
CategroyRouter.post("/category", isAuth, isAdmin, async (req, res) => {
...
CategroyRouter.get("/category/:id", isAuth, isAdmin, async (req, res) => {
  const categ = await Category.findById(req.params.id);
  res.json(categ);
});

CategroyRouter.put("/category/:id", isAuth, isAdmin, async (req, res) => {
  const categ = await Category.findByIdAndUpdate(req.params.id, req.body);
  res.json(categ);
});

CategroyRouter.delete("/category/:id", isAuth, isAdmin, (req, res) => {
